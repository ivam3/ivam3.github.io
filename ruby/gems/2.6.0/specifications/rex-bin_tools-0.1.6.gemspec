# -*- encoding: utf-8 -*-
# stub: rex-bin_tools 0.1.6 ruby lib

Gem::Specification.new do |s|
  s.name = "rex-bin_tools".freeze
  s.version = "0.1.6"

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["David Maloney".freeze]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\nWD9f\n-----END CERTIFICATE-----\n".freeze, "-----BEGIN CERTIFICATE-----\\nsK8EBtz4bEaIcpzrTp3DsLlUo7lOl8oU\n-----END CERTIFICATE-----\n".freeze, "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n".freeze]
  s.date = "2018-10-26"
  s.description = "A suite of tools for analyzing Elf,Mach, and PE format executables to find specific chunks of code.".freeze
  s.email = ["DMaloney@rapid7.com".freeze]
  s.executables = ["msfbinscan".freeze, "msfelfscan".freeze, "msfmachscan".freeze, "msfpescan".freeze]
  s.files = ["bin/msfbinscan".freeze, "bin/msfelfscan".freeze, "bin/msfmachscan".freeze, "bin/msfpescan".freeze]
  s.homepage = "https://github.com/rapid7/rex-bin_tools".freeze
  s.required_ruby_version = Gem::Requirement.new(">= 2.2.0".freeze)
  s.rubygems_version = "3.0.3".freeze
  s.summary = "Ruby Exploitation(rex) Library containing a suite of binary reading and manipulation tools".freeze

  s.installed_by_version = "3.0.3" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_development_dependency(%q<bundler>.freeze, ["~> 1.12"])
      s.add_development_dependency(%q<rake>.freeze, ["~> 10.0"])
      s.add_development_dependency(%q<rspec>.freeze, ["~> 3.0"])
      s.add_runtime_dependency(%q<metasm>.freeze, [">= 0"])
      s.add_runtime_dependency(%q<rex-arch>.freeze, [">= 0"])
      s.add_runtime_dependency(%q<rex-struct2>.freeze, [">= 0"])
      s.add_runtime_dependency(%q<rex-text>.freeze, [">= 0"])
      s.add_runtime_dependency(%q<rex-core>.freeze, [">= 0"])
    else
      s.add_dependency(%q<bundler>.freeze, ["~> 1.12"])
      s.add_dependency(%q<rake>.freeze, ["~> 10.0"])
      s.add_dependency(%q<rspec>.freeze, ["~> 3.0"])
      s.add_dependency(%q<metasm>.freeze, [">= 0"])
      s.add_dependency(%q<rex-arch>.freeze, [">= 0"])
      s.add_dependency(%q<rex-struct2>.freeze, [">= 0"])
      s.add_dependency(%q<rex-text>.freeze, [">= 0"])
      s.add_dependency(%q<rex-core>.freeze, [">= 0"])
    end
  else
    s.add_dependency(%q<bundler>.freeze, ["~> 1.12"])
    s.add_dependency(%q<rake>.freeze, ["~> 10.0"])
    s.add_dependency(%q<rspec>.freeze, ["~> 3.0"])
    s.add_dependency(%q<metasm>.freeze, [">= 0"])
    s.add_dependency(%q<rex-arch>.freeze, [">= 0"])
    s.add_dependency(%q<rex-struct2>.freeze, [">= 0"])
    s.add_dependency(%q<rex-text>.freeze, [">= 0"])
    s.add_dependency(%q<rex-core>.freeze, [">= 0"])
  end
end
