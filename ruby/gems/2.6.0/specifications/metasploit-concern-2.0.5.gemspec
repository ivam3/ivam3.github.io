# -*- encoding: utf-8 -*-
# stub: metasploit-concern 2.0.5 ruby lib

Gem::Specification.new do |s|
  s.name = "metasploit-concern".freeze
  s.version = "2.0.5"

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["Luke Imhoff".freeze]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\nWD9f\n-----END CERTIFICATE-----\n".freeze, "-----BEGIN CERTIFICATE-----\\nsK8EBtz4bEaIcpzrTp3DsLlUo7lOl8oU\n-----END CERTIFICATE-----\n".freeze, "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n".freeze]
  s.date = "2017-06-28"
  s.description = "Automatically includes Modules from app/concerns/<module_with_concerns>/<concern>.rb into <module_with_concerns> to ease monkey-patching associations and validations on ActiveRecord::Base descendents from other gems when layering schemas.".freeze
  s.email = ["luke_imhoff@rapid7.com".freeze]
  s.homepage = "https://github.com/rapid7/metasploit-concern".freeze
  s.licenses = ["BSD-3-clause".freeze]
  s.required_ruby_version = Gem::Requirement.new(">= 2.2.0".freeze)
  s.rubygems_version = "3.0.3".freeze
  s.summary = "Automatically include Modules from app/concerns".freeze

  s.installed_by_version = "3.0.3" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_development_dependency(%q<metasploit-yard>.freeze, [">= 0"])
      s.add_development_dependency(%q<metasploit-erd>.freeze, [">= 0"])
      s.add_runtime_dependency(%q<activemodel>.freeze, ["~> 4.2.6"])
      s.add_runtime_dependency(%q<activesupport>.freeze, ["~> 4.2.6"])
      s.add_runtime_dependency(%q<railties>.freeze, ["~> 4.2.6"])
    else
      s.add_dependency(%q<metasploit-yard>.freeze, [">= 0"])
      s.add_dependency(%q<metasploit-erd>.freeze, [">= 0"])
      s.add_dependency(%q<activemodel>.freeze, ["~> 4.2.6"])
      s.add_dependency(%q<activesupport>.freeze, ["~> 4.2.6"])
      s.add_dependency(%q<railties>.freeze, ["~> 4.2.6"])
    end
  else
    s.add_dependency(%q<metasploit-yard>.freeze, [">= 0"])
    s.add_dependency(%q<metasploit-erd>.freeze, [">= 0"])
    s.add_dependency(%q<activemodel>.freeze, ["~> 4.2.6"])
    s.add_dependency(%q<activesupport>.freeze, ["~> 4.2.6"])
    s.add_dependency(%q<railties>.freeze, ["~> 4.2.6"])
  end
end
